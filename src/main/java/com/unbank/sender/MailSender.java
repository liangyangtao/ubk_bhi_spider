...
public class MailSender {
	...
	public boolean sendTextMail(MailSenderInfo mailInfo) {
		...
		try {
			...
			if (mailInfo.getReceivers() != null
					&& mailInfo.getReceivers().size() > 0) {
				String addressList = getMailList(mailInfo.getReceivers());
				InternetAddress[] receiverAddresses = new InternetAddress()
						.parse(addressList);
				mailMessage.setRecipients(Message.RecipientType.TO,
						receiverAddresses);
			}
			// 邮件抄送人
			if (mailInfo.getCcReceivers() != null
					&& mailInfo.getCcReceivers().size() > 0) {
		...
		try {
			...
			if (mailInfo.getAttachFileNames() != null
				...
				for (int i = 0; i < files.length; i++) {
					...
					// 添加附件的标题
					messageBodyPart.setFileName(MimeUtility.encodeText(source
							.getName()));
